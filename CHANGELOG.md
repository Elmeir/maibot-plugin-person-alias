...
### 重构：注入主语改为 planner 实际可见名，去掉恒等的"称呼="字段

用户连续指出两个格式问题：换名开启时注入"豆包哥：称呼=豆包哥"仍是重复
（v1.4.4 已按"与 planner 可见名比较"过滤）；但格式本身 `主语：称呼=主语`
恒等冗余，且主语取档案名、与消息里实际显示的名字（人工别名来源/overrides
场景）可能不一致。

- **主语 = planner 实际可见名**：换名生效用换名后的名字（含 overrides 与
  "仅人工别名"来源），未生效用消息原始名——注入块与 planner 看到的名字
  永远一致，overrides 名不再断联。
- **格式改为 `主语：别名=…`**：删除恒等的"称呼=主语"字段；别名列表 =
  档案主称呼、档案昵称、各群群名片、人工别名（除主语外，主称呼排首位）。
- 语义简化：别名注入关闭 = 注入块整体停用（纯称呼没有信息量）；
  `injection.enabled` 仍为独立总闸。
- 修复重构引入的 `await ...get()` 优先级错误（协程上调属性）。
- verify_plugin 全量同步（主语/别名断言、无称呼字段断言、停用语义、
  过滤判定）。
