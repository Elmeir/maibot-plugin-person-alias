...
**注入块长什么样**：每行主语是 **planner 在消息里实际看到的名字**（换名生效=换名后的名字；未生效=原群名片），后面列出这个人其余的全部叫法（档案主称呼、档案昵称、各群群名片、人工别名）。例如群名片"群豆包"、档案主称呼"豆包哥"、人工别名"包总"，换名开启时注入：
...
**只注入有增量的内容**：没有其他叫法的人物（注入了也只是重复 planner 已知的名字）不会出现在块里；关闭"注入别名"后整块停用。别名对照的价值在于消息之外的名字来源——记忆/画像原文、知识库文本、换名前落库的历史消息里仍会出现原群名片，这是 LLM 把那些名字对上人的唯一渠道。
