...
- **称呼注入**：向 planner 注入「人物称呼与别名」参考块，每行主语是 planner 在消息里实际看到的名字，后面列出这个人其余的全部叫法（档案主称呼、档案昵称、各群群名片、人工别名）：
  ...
  只注入有增量的内容——没有其他叫法的人物不会出现在块里（注入了也只是重复 planner 已知的名字）。别名对照的价值在于消息之外的名字来源：记忆/画像原文、知识库文本、换名前落库的历史消息里仍会出现原群名片，这是 LLM 把那些名字对上人的唯一渠道。

- **昵称替换**：把运行时显示的 QQ 昵称 / 群名片替换成你自己维护的称呼，两个开关互相独立——关闭注入后换名照常工作。
...
| 注入别名 | 开 | 注入"叫法对照表"；**关闭后注入块整体停用**（消息里的名字 planner 本来就看得到，纯称呼没有信息量） |
