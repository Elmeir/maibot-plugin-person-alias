...
与「昵称替换」的配合——注入块只在"能告诉 planner 它从消息里看不出来的事"时出现：

- **换名开启**：planner 看到的已是画像称呼，纯称呼条目零增量、不再注入；只有开启「注入别名」才注入别名对照——消息之外的名字来源（记忆/画像原文、知识库文本、换名前落库的历史消息）仍会出现原群名片，别名列表是 LLM 对上人的唯一渠道。
- **换名关闭**：planner 看到的是消息里的原群名片，称呼与原名不同的人物注入称呼作对照；相同则跳过。
- 因此"换名开启 + 关闭注入别名"时注入块会整体静默——这是预期行为：此时任何注入都只是重复 planner 已知的信息。
